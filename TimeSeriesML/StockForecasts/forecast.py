...
if len(sys.argv) > 2:
    ...
    EIGHT_LOOKUP_STEP = int(QUART_LOOKUP_STEP/2)
...
def predict_prices(model, data, df2, indexVals):
    predicted_prices = []
    df2['forecast'] = df2['forecast'].astype(float)
    for X in range(min(LOOKUP_STEP*12, len(df2.index)-1)):
        # retrieve the last sequence from data
...
HALF_LOOKUP_STEP_PRICE = df2['forecast'][df2.index[-(LOOKUP_STEP-HALF_LOOKUP_STEP)]]
QUART_LOOKUP_STEP_PRICE = df2['forecast'][df2.index[-(LOOKUP_STEP-QUART_LOOKUP_STEP)]]
EIGHT_LOOKUP_STEP_PRICE = df2['forecast'][df2.index[-(LOOKUP_STEP-EIGHT_LOOKUP_STEP)]]
print(f"Future $ price after {LOOKUP_STEP} days is {future_price:.2f}")
print(f"Future $ price after {HALF_LOOKUP_STEP} days is {HALF_LOOKUP_STEP_PRICE:.2f}")
print(f"Future $ price after {QUART_LOOKUP_STEP} day is {QUART_LOOKUP_STEP_PRICE:.2f}")
print(f"Future $ price after {EIGHT_LOOKUP_STEP} day is {EIGHT_LOOKUP_STEP_PRICE:.2f}")
